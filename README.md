...
# xstartup（关键：dbus-launch 包装 + 禁用锁屏）
...
# 强制禁用 GNOME 锁屏和自动休眠（GNOME 46 兼容）
...
# 设置用户级锁屏禁用（必须通过 VNC 的 DISPLAY 执行，否则 gsettings 无法写入）
sleep 3
su - YOUR_DESKTOP_USER -c 'export DISPLAY=:2; \
...
### 2. VNC 连接后显示锁屏界面，输入密码无反应

**现象**：VNC 连接后看到 GNOME 锁屏（显示时间 + "单击或按键解锁"），点击后出现登录界面但输入密码无法通过（"认证出错"）。

**原因**：
1. xstartup 中直接 `exec gnome-session` 缺少 D-Bus 会话 → 黑屏/秒退
2. GNOME 默认启用锁屏（`lock-enabled=true`）+ 自动空闲锁定（`idle-delay=900`），VNC 断开后重连即触发锁屏
3. 系统级 dconf (`/etc/dconf/db/local.d/`) 只影响新会话的默认值，用户级 gsettings 会覆盖它

**解决**（三层防护缺一不可）：

```bash
# 第 1 层：系统级 dconf 全局默认值（影响所有新用户）
cat > /etc/dconf/db/local.d/01-no-lock << 'EOF'
...
# 第 2 层：xstartup 中启动时执行 gsettings（每次 VNC 启动生效）
# 见步骤 15 的 xstartup 脚本

# 第 3 层：VNC 服务启动后立即设置用户级 gsettings（需 DISPLAY=:2）
su - YOUR_DESKTOP_USER -c 'export DISPLAY=:2; \
  gsettings set org.gnome.desktop.screensaver lock-enabled false; \
  gsettings set org.gnome.desktop.session idle-delay 0'
```

> **关键点**：`gsettings set` 必须在有 X11 DISPLAY 的环境下执行（如 `DISPLAY=:2`），否则报 "无法在没有 X11 $DISPLAY 的情况下自动启动 D-Bus" 且设置不生效。
